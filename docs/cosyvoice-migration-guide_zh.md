...
从 `start_portable.bat serve` 或 `pixi run serve` 启动后，Admin 首页必须自动刷新 API 状态，不要求用户手动点刷新。
...
5. 启动 `start_portable.bat serve` 后打开 Admin 首页，确认 API 状态自动刷新。
